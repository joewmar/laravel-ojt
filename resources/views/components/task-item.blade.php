<div class="rounded-md bg-pink-50 p-4 w-4/5 sm:w-2/5 mx-auto my-2" wire:key="{{ $task->id}}">
    <div class="flex">
        <div class="ml-3 mt-1 flex-1 md:flex md:justify-between">
            <p class="text-sm font-bold text-pink-700">
                {{$task->name}} / {{Carbon\Carbon::parse($task->updated_at)->shiftTimezone('Asia/Manila')->format('d-m-Y H:i')}}
            </p>
